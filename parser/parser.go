...
import (
	...
	"github.com/kemolife/felgo-interpreter/token"
	"strconv"
)

var priorities = map[token.Type]uint8{
	token.EQ:       token.EQUALS,
	token.NotEq:    token.EQUALS,
	...
	token.LPAREN:   token.CALL,
	token.LBRACKET: token.INDEX,
}
...
func New(l *lexer.Lexer) *Parser {
	...
	p.registerPrefix(token.BANG, p.parsePrefixLiteral)
	p.registerPrefix(token.MINUS, p.parsePrefixLiteral)
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	for !p.curTokenIs(token.SEMICOLON) {
		if p.peekToken.Type == token.EOF {
			p.errors = append(p.errors, "Not found semicolon")
			return nil
		}
		p.nextToken()
	}
...
func (p *Parser) parsePrefixLiteral() ast.Expression {
...
func (p *Parser) parseExpression(priority uint8) ast.Expression {
...
func (p *Parser) parseIndexExpression(left ast.Expression) ast.Expression {
	...
	p.nextToken()
	exp.Index = p.parseExpression(token.LOWEST)
...
func (p *Parser) peekPriority() uint8 {
	if p, ok := priorities[p.peekToken.Type]; ok {
		return p
	}
	return token.LOWEST
}
func (p *Parser) currentPriority() uint8 {
